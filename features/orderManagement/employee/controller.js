  ...
  try {
    ...
    if (loginType == 'employee') {
      ...
      if (await bcryptjs.compare(password, user.password)) {
        ...
        let profile =await Employee2.findOne({email:email})
        res.status(200).json({ status: true, role: user.role, token: token, profile:profile })
      } else {
...
exports.addsecretkey = async (req, res) => {
  try {
    const email = req.body.email
    const key = req.body.key
    let profile = await Employee2.findOneAndUpdate(
      { email: email },
      { $set: { secretkey: key } },
      { new: true }
    )
...
exports.addsheetdetails = async (req, res) => {
  try {
    const { newsheet, id } = req.body;
    if (newsheet.sheetname && typeof newsheet.sheetname === 'string') {
      const user = await Employee2.findOne({ _id: id });
      let arraylist = user.sheetlist || [];
      arraylist.push(newsheet.sheetname);
      ...
      const resp = await Employee2.findOneAndUpdate(
        { _id: id },
        { $set: updateField },
        { new: true }
      );
      res.status(200).json({ status: true, profile: resp });
    }
  } catch (err) {
    console.error(err);
    res.status(500).json({ status: false, msg: err.message });
...
exports.addclientid = async (req, res) => {
  try {
    ...
    let resp = await Employee2.findOneAndUpdate(
      { _id: id },
