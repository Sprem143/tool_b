...
exports.sendotp = async (req, res) => {
  try {
    ...
    const mailOptions = {
      ...
      to: email,
      subject: "Your OTP for Admin Sign Up - Gstar E-solution Pvt Ltd",
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px;">
          <h2 style="text-align: center; color: #0d6efd;">Gstar E-solution Pvt Ltd</h2>
          <p>Hello,</p>
          <p>Thank you for choosing Gstar E-solution Pvt Ltd. Please use the following One-Time Password (OTP) to complete your Admin sign-up process:</p>
...
exports.getprofile = async (req, res) => {
    try {
        ...
        let user = await Admin.findOne({ email: admin.email });
        if (!user) {
            return res.status(404).json({ status: false, msg: "Admin not found" });
        }
        user = user.toObject()
        delete user.password;
        return res.status(200).json({ status: true, admin: user });
...
function getWelcomeEmailTemplate(name, email, password) {
    ...
    <div style="font-family: Arial, sans-serif; max-width:600px; margin:auto; border:1px solid #e0e0e0; border-radius:10px; overflow:hidden;">
      ...
      <div style="padding:20px;">
        ...
        <p>We are excited to welcome you to <strong>Gster E-Solution Pvt Ltd</strong> as a Order Management Admin!</p>
