const { belk, handleoutofstock, saveData, boscov, academy, walmart } = require('./util')
...
exports.thread1 = async (req, res) => {
    try {
        ...
        // -----------HANDLE WALMART  URL----
            ...
            let resp = await walmart(url, account)
        return  resp? res.status(200).json({status:true}) : res.status(500).json({status:false})
        }
        // -------------handle academy url -------
